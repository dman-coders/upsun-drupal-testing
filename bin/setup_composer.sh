...
add_sample_run_tests_as_composer_scripts() {
  ...
  the_script="cd ${DOCROOT} && XDEBUG_MODE=off phpunit --configuration ${phpunit_xml_file} ${the_test}"

  # Check if script already exists using composer config
  if composer config scripts.${the_alias} &>/dev/null; then
    log_info "Script '${the_alias}' already exists, skipping."
  else
    composer config scripts.${the_alias} "${the_script}"
